...
// ─── Template: Agent password reset ───────────────────────────────────────
export function agentPasswordResetEmailHtml({ firstName, name, resetUrl }) {
  const bodyHtml = `
    ${eyebrow('Password Reset')}
    ${heading(`Reset your password, ${firstName}`)}
    ${divider}
    ${body(`We received a request to reset the password for your <strong style="color:${C.white};">BYD Properties</strong> agent account. Click the button below to choose a new password.`)}
    ${body(`This link is valid for <strong style="color:${C.gold};">1 hour</strong>. If you did not request a password reset, you can safely ignore this email — your password will remain unchanged.`)}

    ${infoBox(`
      <p style="margin:0 0 14px;font-size:11px;font-weight:700;
                letter-spacing:0.15em;text-transform:uppercase;
                color:${C.gold};">Account Details</p>
      ${infoRow('Name', name)}
    ...
    <p style="margin:36px 0 0;font-size:12px;color:${C.textSub};line-height:1.7;">
      ...
      <a href="${resetUrl}" style="color:${C.textMuted};word-break:break-all;">${resetUrl}</a>
    </p>
  `;
  return wrapEmail({ title: 'Reset Your Password — BYD Properties', bodyHtml });
}
